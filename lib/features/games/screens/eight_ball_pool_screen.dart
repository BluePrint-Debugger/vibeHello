...
import '../services/eight_ball_pool_service.dart';
...
class _EightBallPoolScreenState extends State<EightBallPoolScreen> {
  final EightBallPoolService _service = EightBallPoolService();
          ...
          return Padding(
            ...
            child: Column(
              children: [
                ...
                _PoolTableSimple(),
                const SizedBox(height: 30),
                _TurnIndicator8Ball(isMyTurn: isMyTurn),
                const SizedBox(height: 20),
                _ShootButton(
                  onShoot: isMyTurn ? () => _handleShoot() : null,
                  isMyTurn: isMyTurn,
                ),
              ],
            ),
...
class _PoolTableSimple extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Container(
      width: 400,
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: List.generate(16, (index) {
        final isPotted = index.isEven; // simplified
        return Container(
          width: 20,
          height: 20,
          margin: const EdgeInsets.symmetric(horizontal: 2),
          decoration: BoxDecoration(
            color: isPotted ? Colors.transparent : Colors.white,
...
class _TurnIndicator8Ball extends StatelessWidget {
  final bool isMyTurn;

  const _TurnIndicator8Ball({required this.isMyTurn});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(
        isMyTurn ? 'Your turn - Shoot 8 Ball!' : 'Opponent\'s turn',
